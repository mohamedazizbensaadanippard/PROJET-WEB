...
foreach ($produits as $p) {
    $total += (float)$p['prix'] * $panier[$p['id']];
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($nom != '' && $email != '') {
        try {
            ...
            $reqCmd = $pdo->prepare("INSERT INTO commandes (user_id, nom_client, email_client, telephone, total) VALUES (?, ?, ?, ?, ?) RETURNING id");
            $reqCmd->execute([$_SESSION['user']['id'], $nom, $email, $tel, $total]);
            ...
            foreach ($produits as $p) {
                $qte = $panier[$p['id']];
                if ($p['stock'] < $qte) {
                    throw new Exception("Stock insuffisant pour " . $p['nom']);
                }
...
<!DOCTYPE html>
<html lang="fr">
<head><meta charset="UTF-8"><title>Commande - GearHub</title><link rel="stylesheet" href="style.css"></head>
<body>
<?php include 'header.php'; ?>
<main class="page">
    <section class="haut-page"><div><p class="mini-title">Commande</p><h1>Validation</h1></div></section>
    ...
    <?php else: ?>
        <section class="commande-page">
            <form method="post" class="bloc form-client">
                ...
                <input type="text" name="nom" required value="<?= htmlspecialchars($_SESSION['user']['nom']) ?>">
                <label>Email</label>
                <input type="email" name="email" required value="<?= htmlspecialchars($_SESSION['user']['email']) ?>">
            ...
            <aside class="resume bloc">
                ...
                <p>Le stock diminue automatiquement après la commande.</p>
            </aside>
        </section>
    <?php endif; ?>
</main>
